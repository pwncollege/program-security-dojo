...
int main(int argc, char **argv, char **envp)
{
    ...
    shellcode = mmap((void *)0x2df2d000, 0x4000, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_PRIVATE|MAP_ANON, 0, 0);
    assert(shellcode == (void *)0x2df2d000);
    printf("Mapped 0x4000 bytes for shellcode at %p!\n", shellcode);

    puts("Reading 0x4000 bytes from stdin.\n");
    shellcode_size = read(0, shellcode, 0x4000);
    ...
    puts("This challenge is about to close stderr, which means that you will not be able to use file descriptor 2 for output.\n");
    assert(fclose(stderr) == 0);

    puts("This challenge is about to close stdout, which means that you will not be able to use file descriptor 1 for output. You");
