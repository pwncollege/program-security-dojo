...
int main(int argc, char **argv, char **envp)
{
    ...
    shellcode_mem = mmap((void *) 0x1618b000, 0x1000, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_PRIVATE|MAP_ANON, 0, 0);
    printf("[LEAK] Mapping shellcode memory at %p!\n", shellcode_mem);
    assert(shellcode_mem == (void *) 0x1618b000);
    ...
    puts("instruction is an `INT 3`, which is an interrupt to call into the debugger. You must avoid these modifications in your");
    puts("shellcode.\n");
